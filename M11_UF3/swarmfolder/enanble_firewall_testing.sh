...
#
### Default DROP
echo iptables -P INPUT $kill
echo iptables -P OUTPUT $kill
echo iptables -P FORWARD $kill
#
### Enable communication

echo iptables -A INPUT -m state --state ESTABLISHED,RELATED -j $allow
echo iptables -A OUTPUT -m state --state ESTABLISHED,RELATED -j $allow
echo iptables -A FORWARD -m state --state ESTABLISHED,RELATED -j $allow
...
echo iptables -A FORWARD -p tcp --dport ${port_dict["HTTP"]} -m state --state NEW -j $allow
echo iptables -A FORWARD -p tcp --dport ${port_dict["HTTPS"]} -m state --state NEW -j $allow
echo iptables -A FORWARD -p tcp --dport ${port_dict["DNS"]} -m state --state NEW -j $allow
echo iptables -A FORWARD -p udp --dport ${port_dict["DNS"]} -m state --state NEW -j $allow
...
echo iptables -t nat -A POSTROUTING -s $net_emp -o $iface_pub -j MASQUERADE
echo iptables -t nat -A POSTROUTING -s $net_dmz -o $iface_pub -j MASQUERADE
...
for service in "${!dmz_public_enabled[@]}"; do
  if [[ 1 -eq "${dmz_public_enabled[$service]}" ]] ; then
      echo iptables -t nat -A PREROUTING -i $iface_pub -p tcp --dport "${port_dict[$service]}" -j DNAT --to "${ip_dmz}:${port_dict[$service]}"
...
for service in "${!port_dict[@]}"; do
  if [[ 1 -eq "${web_public_enabled[$service]}" ]] ; then
      echo iptables -t nat -A PREROUTING -i $iface_pub -p tcp --dport "${port_dict[$service]}" -j DNAT --to "${ip_web}:${port_dict[$service]}"
...
# WEB to DB  ENABLE
service=$db_name
echo iptables -A FORWARD -s $ip_web -d $ip_db -p tcp --dport "${port_dict[$service]}" -j $allow
...
# DB to WEB  ENABLE
echo iptables -A FORWARD -s $ip_db -d $ip_web -p tcp --dport "${port_dict[$service]}" -j $allow
...
for service in "${!dmz_local_enabled[@]}"; do
  if [[ 1 -eq "${dmz_local_enabled[$service]}" ]] ; then
      echo iptables -A FORWARD -s $net_emp -d $net_dmz -p tcp --dport "${port_dict[$service]}" -m state --state NEW -j $allow
...
#printf "." && sleep "0.2" && #printf "." && sleep "0.2" && #printf "." && sleep "0.2" && #printf "." && sleep "0.2" && #printf "." && sleep "0.2" && #printf "." && sleep "0.2" && #printf "." && sleep "0.2"
#printf "\n"
echo iptables -L OUTPUT
#printf "." && sleep "0.2" && #printf "." && sleep "0.2" && #printf "." && sleep "0.2" && #printf "." && sleep "0.2" && #printf "." && sleep "0.2" && #printf "." && sleep "0.2" && #printf "." && sleep "0.2"
#printf "\nPrinting current FORWARD rules:\n"
#printf "." && sleep "0.2" && #printf "." && sleep "0.2" && #printf "." && sleep "0.2" && #printf "." && sleep "0.2" && #printf "." && sleep "0.2" && #printf "." && sleep "0.2" && #printf "." && sleep "0.2"
#printf "\n"
echo iptables -L FORWARD
sleep 2
#printf "\n(END)\n"
...
#
## Enable pings
#echo iptables -A INPUT -p icmp  -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT
#echo iptables -A OUTPUT -p icmp  -m state --state NEW,ESTABLISHED,RELATED -j ACCEPT