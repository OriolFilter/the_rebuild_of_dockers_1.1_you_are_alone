...
if [[ "$1" -eq 1 ]]; then # 0 is NO fer DPIPE, 1 ES fer DPIPE
	printf "S'esta iniciat el server VDE amb el port $LPORT\n";
	dpipe vde_plug -p $PORT /tmp/$SWITCHSERVER1 = nc -l -u -p $LPORT & disown;
	printf "S'ha finaliztat iniciar el server switch VDE\n";
fi
